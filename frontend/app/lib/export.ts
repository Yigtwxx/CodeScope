...
export function exportAsJSON(conversation: Conversation): void {
  ...
  downloadFile(json, `${sanitizeFilename(conversation.title)}.json`, 'application/json')
}
...
export async function exportAsPDF(conversation: Conversation): Promise<void> {
  ...
  for (const msg of conversation.messages) {
    ...
    pdf.setFontSize(12)
    pdf.setFont('helvetica', 'bold')
    // The built-in helvetica face has no glyphs outside Latin-1, so the label
    // stays plain text.
    pdf.text(msg.role === 'user' ? 'User' : 'Assistant', margin, y)
    y += lineHeight
...
/**
 * Reduce a conversation title to something every filesystem accepts.
 *
 * Letters and digits from any script are kept — an ASCII-only rule turned a
 * Turkish title into a row of underscores. Runs of anything else collapse to a
 * single separator.
 */
